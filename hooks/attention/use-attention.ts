...
export function useAcknowledgeAttentionItem() {
  ...
  return useMutation({
    mutationFn: ({ id, note }: { id: string; note?: string }) =>
      attentionAPI.acknowledgeItem(id, note, organizationId),
    onSuccess: () => {
      invalidate();
      toast.success(
        "Alerta assumido. A equipe agora sabe que você está cuidando dele.",
      );
    },
    onError: (error: Error) =>
      toast.error(error.message || "Não foi possível assumir o alerta."),
  });
}
...
export function useSnoozeAttentionItem() {
  ...
  return useMutation({
    ...
    onError: (error: Error) =>
      toast.error(error.message || "Não foi possível adiar o alerta."),
  });
}
...
export function useCreateAttentionPolicy() {
  ...
  return useMutation({
    mutationFn: (input: CreateAttentionPolicyInput) =>
      attentionAPI.createPolicy(input, organizationId),
    onSuccess: () => {
...
export function useUpdateAttentionPolicy() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      id: string;
      input: UpdateAttentionPolicyInput;
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["home"] });
      toast.success(
        "Regra atualizada. Ciclos existentes mantêm a versão original.",
      );
    },
    onError: (error: Error) =>
      toast.error(error.message || "Não foi possível atualizar a regra."),
  });
}
...
export function useUpdateAttentionSettings() {
  ...
  return useMutation({
    mutationFn: (input: UpdateAttentionSettingsInput) =>
      attentionAPI.updateSettings(input, organizationId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["attention", "settings"] });
      queryClient.invalidateQueries({ queryKey: ["attention", "items"] });
      queryClient.invalidateQueries({ queryKey: ["attention", "summary"] });
      queryClient.invalidateQueries({ queryKey: ["schedule-capabilities"] });
    ...
    onError: (error: Error) =>
      toast.error(
        error.message || "Não foi possível atualizar a segurança global.",
      ),
  });
}
