...
func TestEvolutionGoEdgePersistsCTWAAttributionAcrossManagedRetryLifecycle(t *testing.T) {
	...
	if autoReply < 0 || effectsComplete < 0 {
		t.Fatal("normal WhatsApp messages must await idempotent auto-reply enqueue independently before completing the effect ledger")
	}
}
