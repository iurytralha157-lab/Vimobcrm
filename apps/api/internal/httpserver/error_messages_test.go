...
func TestUserFacingErrorMessage(t *testing.T) {
	tests := []struct {
		...
		{name: "WhatsApp distribution input", code: "invalid_round_robin_input", original: "invalid round robin input: active WhatsApp message distribution requires the simple strategy", status: http.StatusBadRequest, want: "Para ativar a campanha do WhatsApp, use uma fila dedicada com distribuição sequencial, adicione os corretores individualmente, desative o check-in e a redistribuição automática, e ative “Ignorar agenda nesta fila”."},
