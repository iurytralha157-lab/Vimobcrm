...
const image = {
  ...
  mimeType: 'image/png',
  buffer: Buffer.from(
    'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Y9ZbQAAAABJRU5ErkJggg==',
    'base64',
  ),
};
...
async function expectPropertyFormSegmentedTabs(page: Page) {
  const tabList = page.getByRole('tablist', {
    name: 'Etapas do formulário do imóvel',
  });
  await expect(tabList).toBeVisible();
  await expect(tabList.getByRole('tab')).toHaveCount(
    PROPERTY_FORM_TAB_NAMES.length,
  );
  for (const name of PROPERTY_FORM_TAB_NAMES) {
  ...
  await expect(tabList.locator('xpath=..')).toHaveAttribute(
    'data-collapse',
    'compact',
  );
  await expect(tabList).toHaveAttribute('data-responsive-tab-scroll', /.*/);
...
async function fillCreationForm(page: Page, title: string, suffix: string) {
  ...
  const currencies = page.locator(
    '[data-tour="property-values-section"] input[inputmode="decimal"]',
  );
  const values = [
    '1.500,80',
    '850,45',
  ...
  for (let index = 0; index < values.length; index += 1)
    await currencies.nth(index).fill(values[index]);
  ...
  const characteristicSelects = page.locator(
    '[data-tour="property-characteristics-section"] button[role="combobox"]',
  );
  ...
  await page
    .getByPlaceholder('Observações internas...')
    .fill(`Interno ${suffix}`);

  await page.getByRole('tab', { name: 'Extras', exact: true }).click();
  await expect(
    page.getByText('Detalhes Extras do Imóvel', { exact: true }),
  ).toBeVisible();
  ...
  const fileInputs = page.locator(
    '[data-tour="property-media-section"] input[type="file"]',
  );
  ...
  await page
    ...
    .fill('https://youtube.com/watch?v=e2e');
  await page
    .getByPlaceholder('Observações e descrição usadas dentro do CRM...')
    .fill(`Descricao interna ${suffix}`);
  await page
    .getByPlaceholder('Texto comercial que será exibido no site público...')
    .fill(`Descricao publica ${suffix}`);
  ...
  await expect(page.getByTestId('property-publication-guidance')).toContainText(
    'será salvo fora do site',
  );
  await expect(page.getByRole('switch', { name: 'Anunciar' })).toHaveCount(0);
  await page.getByRole('switch', { name: 'Imóvel em Destaque' }).click();
  await page.getByRole('switch', { name: 'Placa no Local' }).click();
  ...
  await page
    .getByPlaceholder('Condições especiais...')
    .fill(`Comissao ${suffix}`);
  ...
  const confidentialCodes = page.locator(
    '[data-tour="property-confidential-section"] input[placeholder="Código"]',
  );
  ...
  await page
    .getByPlaceholder('Observações sobre documentação...')
    .fill(`Documentos ${suffix}`);
}
...
async function editPropertyThroughUI(
  page: Page,
  property: { id: string; title: string; suffix: string },
) {
  ...
  await page
    ...
    .fill(`Proprietario editado ${property.suffix}`);
  await openTab(
    page,
    'Dados do imóvel',
    '[data-tour="property-structure-section"]',
  ...
  await openTab(page, 'Valores', '[data-tour="property-values-section"]');
  await page
    .locator('[data-tour="property-values-section"] input[inputmode="decimal"]')
    .nth(0)
    .fill('9.876,54');
  await openTab(
    page,
  ...
  await page
    .getByPlaceholder('Observações internas...')
    .fill(`Interno editado ${property.suffix}`);
  await page.getByRole('tab', { name: 'Extras', exact: true }).click();
  ...
  await expect(page.getByTestId('property-publication-guidance')).toContainText(
    'Use a Central de Publicação',
  );
  await expect(page.getByRole('switch', { name: 'Anunciar' })).toHaveCount(0);
  await page.getByRole('switch', { name: 'Super Destaque' }).click();
  await openTab(
    page,
    'Comissões',
    '[data-tour="property-commissions-section"]',
  ...
  await page
    .getByPlaceholder('Condições especiais...')
    .fill(`Comissao editada ${property.suffix}`);
  await openTab(
    page,
    'Confidencial',
    '[data-tour="property-confidential-section"]',
  ...
  await page
    .getByPlaceholder('Observações sobre documentação...')
    .fill(`Documentos editados ${property.suffix}`);
...
async function setPropertyManage(
  adminPage: Page,
  userID: string,
  allowed: boolean,
) {
...
async function runLifecycle(browser: Browser, key: E2EUserKey) {
  ...
  try {
    if (key !== 'admin') {
      ...
      const denied = await authenticatedAPIRequest(
        actorPage,
        'POST',
        '/v1/properties',
        { title: 'Negado E2E' },
      ...
      await signInAs(actorPage, key);
      await expect
        .poll(async () => (await fetchTenantContext(actorPage)).permissions, {
          timeout: 15_000,
        })
        .toContain('property_manage');
    ...
    if (userID && adminContext) {
      ...
      if (pages[0] && !pages[0].isClosed())
        await setPropertyManage(pages[0], userID, false).catch(() => undefined);
    }
    await actorContext?.close();
...
test.describe.serial('cadastro e edicao completa de imoveis por perfil', () => {
  ...
  test('usuario bloqueado por padrao e funcional com property_manage', async ({
    browser,
  }) => runLifecycle(browser, 'user'));
});
