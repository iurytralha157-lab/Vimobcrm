  ...
  return {
    data: {
      property: {
        ...
        faixa_valor_imovel: "R$ 400 mil a R$ 500 mil",
      ...
      summary: {
        ...
        checklist: [
          { code: "title", label: "Titulo preenchido", resolved: true },
        ],
        counts: {
          offers: 0,
          owners: 1,
...
test("workspace offer precondition requires an RFC3339 timestamp with offset", () => {
  assert.equal(
    propertyOfferUpsertInputSchema.safeParse({
      status: "draft",
      price: null,
      currency: "BRL",
...
test("key checkout requires a holder and location change requires a destination", () => {
  assert.equal(
    propertyKeyMovementInputSchema.safeParse({ movement_type: "checkout" })
      .success,
    false,
  );
  assert.equal(
    propertyKeyMovementInputSchema.safeParse({
...
test("ownership create requires exactly one owner source", () => {
  ...
  assert.equal(
    propertyOwnershipCreateInputSchema.safeParse(relationship).success,
    false,
  );
  assert.equal(
    propertyOwnershipCreateInputSchema.safeParse({
      ...relationship,
      owner_id: OWNER_ID,
...
test("ownership owner update has an independent concurrency precondition", () => {
  ...
  assert.equal(
    propertyOwnershipUpdateInputSchema.safeParse({
      ...base,
      owner: { name: "Maria" },
    }).success,
    false,
...
test("assets accept only HTTP locators and one source", () => {
  ...
  assert.equal(
    propertyAssetCreateInputSchema.safeParse({
      ...base,
...
test("asset upload discard accepts only one bounded canonical path candidate", () => {
  assert.deepEqual(
    propertyAssetUploadDiscardInputSchema.parse({
      storage_path: `orgs/${ORGANIZATION_ID}/properties/${PROPERTY_ID}/${OWNER_ID}/photo.jpg`,
    }),
    {
      storage_path: `orgs/${ORGANIZATION_ID}/properties/${PROPERTY_ID}/${OWNER_ID}/photo.jpg`,
    },
  ...
  assert.equal(
    propertyAssetUploadDiscardInputSchema.safeParse({
      storage_path: "",
      extra: true,
    }).success,
    false,
...
test("asset ordering is bounded to 200 unique rows", () => {
  ...
  assert.equal(
    propertyAssetOrderInputSchema.safeParse({ items: [item, item] }).success,
    false,
  );
  assert.equal(
    propertyAssetOrderInputSchema.safeParse({
...
test("workspace response keeps an explicit safe projection of database rows", () => {
  const parsed = apiPropertyWorkspaceResponseSchema.parse(
    workspaceResponseFixture(),
  );
  ...
  assert.equal("faixa_valor_imovel" in parsed.data.property, false);
  assert.equal("renda_familiar" in parsed.data.property, false);
  assert.equal("is_demo" in parsed.data.property, false);
...
test("workspace responsible compatibility alias accepts only UUID or null", () => {
  ...
  (missingResponsible.data.property as Record<string, unknown>).cadastrado_por =
    null;
  assert.equal(
    apiPropertyWorkspaceResponseSchema.parse(missingResponsible).data.property
      .cadastrado_por,
    null,
  );
  ...
  for (const invalid of ["", "captador legado"]) {
    ...
    assert.equal(
      apiPropertyWorkspaceResponseSchema.safeParse(fixture).success,
      false,
    );
  }
});
...
test("workspace response accepts legacy and degraded capability metadata", () => {
  const legacy = apiPropertyWorkspaceResponseSchema.parse(
    workspaceResponseFixture(),
  );
...
test("workspace response exposes a strict development backlink without breaking legacy payloads", () => {
  const legacy = apiPropertyWorkspaceResponseSchema.parse(
    workspaceResponseFixture(),
  );
  assert.equal(legacy.data.development_link, null);
  assert.equal(legacy.meta.development_link_available, false);

  const fixture = workspaceResponseFixture();
  const linked = apiPropertyWorkspaceResponseSchema.parse({
    ...fixture,
    data: {
      ...fixture.data,
      development_link: {
    ...
    meta: {
      ...fixture.meta,
      development_link_available: true,
    },
  });
  assert.equal(linked.data.development_link?.development.code, "DEV-001");
  assert.equal(linked.data.development_link?.unit.code, "T01-0101");

  const degraded = apiPropertyWorkspaceResponseSchema.parse({
    ...fixture,
    meta: {
      ...fixture.meta,
      development_link_available: false,
    },
  });
  assert.equal(degraded.data.development_link, null);
  assert.equal(degraded.meta.development_link_available, false);
});
...
test("workspace response preserves owner and ownership notes only for managers", () => {
  ...
  assert.equal(
    parsed.data.property.owner_email,
    "must-not-enter-cache@example.com",
  );
  assert.equal(
    parsed.data.property.comentarios_internos,
    "must not enter the workspace projection",
  );
  assert.equal(parsed.data.property.numero_matricula, "MAT-001");
  assert.equal(parsed.data.property.aprovacao_ambiental, "aprovada");
  assert.equal(parsed.data.property.faixa_valor_imovel, "R$ 400 mil a R$ 500 mil");
  assert.equal(parsed.data.property.renda_familiar, 12000);
  assert.equal(parsed.data.property.is_demo, true);
});
...
test("property workspace keeps compatibility details internal and exposes carousel controls", () => {
  ...
  const overviewSource = readFileSync(
    "components/features/properties/detail/PropertyWorkspaceOverview.tsx",
    "utf8",
  );
  const sectionsSource = readFileSync(
    "components/features/properties/detail/PropertyWorkspaceSections.tsx",
    "utf8",
  );
