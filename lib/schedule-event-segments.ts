...
export function splitScheduleEventByDay<TEvent extends ScheduleEventInterval>(
  ...
  while (segmentStart.getTime() < eventEnd.getTime()) {
    const zonedDateKey = timeZone
      ? getScheduleCivilDateKey(segmentStart, timeZone)
      : null;
    const nextCivilDate = zonedDateKey
      ? getNextScheduleCivilDate(zonedDateKey)
      : null;
    const zonedNextDay =
      nextCivilDate && timeZone
        ? getScheduleCivilDayStart(nextCivilDate, timeZone)
        : null;
    const nextDay = zonedNextDay || startOfNextLocalDay(segmentStart);
    const segmentEnd =
      eventEnd.getTime() < nextDay.getTime() ? eventEnd : nextDay;
    const dateKey = zonedDateKey || localDateKey(segmentStart);
