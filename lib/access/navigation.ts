...
export function selectMobileNavigationItems<T extends NavigationAccessItem>(items: readonly T[]) {
  ...
  const secondary = secondaryItem ? selectMobileNavigationTarget(secondaryItem) : undefined

  return { primary, secondary }
}

function selectMobileNavigationTarget<T extends NavigationAccessItem>(item: T): T {
...
export function resolveMobileFabAction({
  ...
  if (pathname === '/properties') return hasPermission('property_manage') ? 'property' : null
  if (pathname === '/agenda') return hasPermission('schedule_manage') ? 'schedule' : null
  ...
  if (pathname === '/settings') {
    return (tab === 'team' || tab === 'users') && hasPermission('users_manage') ? 'user' : null
  }

  return MOBILE_LEAD_CREATE_PATHS.has(pathname) && hasPermission('lead_create') ? 'lead' : null
}
...
export function getNavigationLocationKey(
  ...
  const hash = currentHash
    ...
    : ''
  const routeKey = tab ? `${pathname}?tab=${encodeURIComponent(tab)}` : pathname
...
export function isNavigationPathActive(
  ...
  const currentParams = typeof search === 'string' ? new URLSearchParams(search) : search
