...
} from "./detail/PropertyOwnershipDialog";
...
function collectPropertyImages(
  ...
  return Array.from(
    new Set(hasCanonicalPhotos ? normalizedPhotos : safeLegacyImages),
  );
}
...
export function PropertyWorkspaceScreen({
  initialTab = "overview",
}: PropertyWorkspaceScreenProps) {
  ...
  const [activeTab, setActiveTab] =
    useState<PropertyWorkspaceTab>(initialTab);
  ...
  const [movementType, setMovementType] =
    useState<ActionableKeyMovement>("checkout");
  const [selectedKey, setSelectedKey] = useState<PropertyWorkspaceKey | null>(
    null,
  );
  ...
  const [selectedOwnership, setSelectedOwnership] =
    useState<PropertyWorkspaceOwnership | null>(null);

  const response = workspaceQuery.data;
  const workspace = response?.data;
  const property = workspace?.property;
  const isEditAccessReady = isPropertyEditAccessReady({
    isEditing: true,
    ...
    () =>
      property
        ? collectPropertyImages(
            property,
            orderedAssets,
            Boolean(response?.meta.has_asset_photos),
          )
        : [],
    [property, orderedAssets, response?.meta.has_asset_photos],
  );
  const selectedOffer =
    workspace?.offers.find((offer) => offer.offer_type === offerType) ?? null;
    ...
    return (
      <AppLayout title="Ficha do imóvel" borderless>
        <div className="property-workspace-surface mx-auto flex min-h-[55vh] max-w-2xl items-center p-6 text-[12px] font-light [&_button]:rounded-[6px] [&_button]:text-[12px] [&_button]:font-light">
          <Alert
            variant="destructive"
            className="rounded-[8px] border-0 bg-destructive/10"
          >
            ...
            <AlertDescription className="mt-2">
              {workspaceQuery.error instanceof Error
                ...
                : "O imóvel não existe ou não está disponível para o seu perfil."}
  ...
  const address = [
    ...
    .join(", ");
  const primaryOffer =
    workspace.offers.find((offer) => offer.status === "active") ??
    workspace.offers[0];
  const title =
    property.title ||
  ...
  const submitKey = async (
    input: Parameters<typeof keyMutation.mutateAsync>[0],
  ) => {
  ...
  const openMovement = (
    propertyKey: PropertyWorkspaceKey,
    type: ActionableKeyMovement,
  ) => {
  ...
  const openOwnership = (
    ownership: PropertyWorkspaceOwnership | null = null,
  ) => {
  ...
  const submitEndOwnership = async (
    input: Parameters<typeof endOwnershipMutation.mutateAsync>[0]["input"],
  ) => {
    if (!selectedOwnership) return;
    await endOwnershipMutation.mutateAsync({
      ...
      <div className="property-workspace-surface min-h-full bg-transparent text-[12px] font-light [&_button]:rounded-[6px] [&_button]:text-[12px] [&_button]:font-light">
        <div className="mx-auto max-w-[1500px] space-y-6 py-2">
          <header className="space-y-4">
                    ...
                    >
                      {propertySiteUrl && (
                        <>
                          ...
                          >
                            <Share2
                              className={PROPERTY_ACTION_MENU_ICON_CLASS}
                            />
                            Compartilhar link
                          </DropdownMenuItem>
                          <DropdownMenuItem
                        ...
                        <>
                          ...
                          <DropdownMenuItem
                            ...
                            onSelect={() => {
                              ...
                              router.replace(
                                `/properties/${property.id}?tab=publication`,
                                { scroll: false },
                              );
                            }}
                          >
            ...
            {developmentLink && response.meta.can_manage && (
              <button
                ...
                onClick={() =>
                  router.push(
                    `/properties/developments/${developmentLink.development.id}`,
                  )
                }
                className="group flex w-full items-center gap-3 rounded-[8px] border-0 bg-[var(--app-surface-soft)] px-3 py-2.5 text-left shadow-none transition-colors hover:bg-[var(--app-surface-hover)] sm:px-4"
              >
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[6px] bg-primary/10 text-primary">
                  <Building2 className="h-4 w-4" />
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block text-[10px] font-light uppercase tracking-[0.08em] text-[var(--app-text-tertiary)]">
                    Vinculado ao empreendimento
                  </span>
                  <span className="mt-0.5 flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[11px] font-light text-[var(--app-text-secondary)]">
                    <span className="font-normal text-[var(--app-text-primary)]">
                      {developmentLink.development.name}
                    </span>
                    <span className="font-mono text-[10px] text-primary">
                      {developmentLink.development.code}
                    </span>
                    <span aria-hidden="true">›</span>
                    <span>{developmentLink.phase.name}</span>
                    <span aria-hidden="true">›</span>
                    <span>{developmentLink.building.name}</span>
                    {developmentLink.floor_plan && (
                      <>
                        <span aria-hidden="true">›</span>
                        <span>{developmentLink.floor_plan.name}</span>
                      </>
                    )}
                    <span aria-hidden="true">›</span>
                    <span>Unidade {developmentLink.unit.unit_number}</span>
                  </span>
                </span>
                <span className="hidden shrink-0 items-center gap-1 text-[11px] font-light text-primary sm:flex">
                ...
                <CardContent className="flex flex-1 flex-col p-4 sm:p-5">
                  ...
                  <div className="mt-auto pt-5">
                    <div className="flex items-center gap-2 text-[11px] font-light text-muted-foreground">
                      <span className="font-normal text-[var(--app-text-primary)]">
                        {workspace.summary.completeness_score}%
                      </span>
                      de completude
                    </div>
                  </div>
          ...
          <Tabs
            value={activeTab}
            onValueChange={(value) => {
              const nextTab = value as PropertyWorkspaceTab;
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <FileText
                    aria-hidden="true"
                    className={WORKSPACE_TAB_ICON_CLASS}
                  />
            ...
            <TabsContent value="commercial" className="space-y-6">
              <PropertyWorkspaceCommercialRegistration property={property} />

              {normalizedResourcesAvailable && (
                <div className="grid gap-4 xl:grid-cols-3">
                  {(["sale", "rent", "seasonal"] as PropertyOfferType[]).map(
                    (type) => {
                      const offer = workspace.offers.find(
                        (item) => item.offer_type === type,
                      );
                      return (
                        <Card
                          key={type}
                          className="rounded-[8px] border-0 bg-[var(--app-surface-solid)] shadow-none"
                        >
                          ...
                          <CardContent className="space-y-3">
                            <div className="rounded-[6px] bg-[var(--app-surface-soft)] p-3">
                              ...
                              <p className="mt-1 text-[10px] font-light text-muted-foreground">
                                {offer?.price_period
                                  ? OFFER_PERIOD_LABELS[offer.price_period]
                              ...
                              <>
                                <dl className="grid grid-cols-2 gap-2 text-[10px] font-light">
                                  ...
                                  <div>
                                    ...
                                    <dd className="mt-0.5 font-normal">
                                      {formatDate(offer.published_at, true)}
                                    </dd>
                                  </div>
                                  <div>
                                    <dt className="text-muted-foreground">
                                    ...
                                    <dd className="mt-0.5 font-normal">
                                      {formatDate(offer.completed_at, true)}
                                    </dd>
                                  </div>
                                  <div>
                                    <dt className="text-muted-foreground">
                                  ...
                                  <div>
                                    ...
                                    <dd className="mt-0.5 font-normal">
                                      {formatDate(offer.updated_at, true)}
                                    </dd>
                                  </div>
                                </dl>
                                {Object.keys(offer.terms).length > 0 && (
                                  <div className="rounded-[6px] bg-[var(--app-surface-soft)] p-3 text-[11px] font-light text-muted-foreground">
                                    Condições da oferta:{" "}
                                    <span className="font-normal text-[var(--app-text-primary)]">
                                      {Object.keys(offer.terms).length} campos
                                      estruturados
                                    </span>
                                  </div>
                                )}
                                {Object.keys(offer.metadata).length > 0 && (
                                  <div className="rounded-[6px] bg-[var(--app-surface-soft)] p-3 text-[11px] font-light text-muted-foreground">
                                    Metadados da oferta:{" "}
                                    <span className="font-normal text-[var(--app-text-primary)]">
                                      {Object.keys(offer.metadata).length}{" "}
                                      campos registrados
                                    </span>
                                  </div>
                                )}
                                <p className="break-all font-mono text-[9px] font-light text-muted-foreground">
                                  Oferta: {offer.id}
                                </p>
                              </>
                            )}
                  ...
                  <CardContent className="p-5">
                    <dl className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
                      {[
                        [
                          "Publicado no site",
                          (property.published_on_site ?? property.anunciar) ==
                          null
                            ? "Não informado"
                            : (property.published_on_site ?? property.anunciar)
                              ? "Sim"
                        ...
                        [
                          "Destaque",
                          (property.is_featured ?? property.destaque) == null
                            ? "Não informado"
                            : (property.is_featured ?? property.destaque)
                              ? "Sim"
                              : "Não",
                        ],
                        [
                          "Super destaque",
                          property.super_destaque == null
                            ? "Não informado"
                            : property.super_destaque
                              ? "Sim"
                              : "Não",
                        ],
                        [
                          ...
                          property.address_visibility ||
                            property.public_address_visibility ||
                            "Não informado",
                        ],
                      ].map(([label, value]) => (
                        <div
                        ...
                        >
                          <dt className="text-[10px] font-light uppercase tracking-[0.08em] text-muted-foreground">
                            {label}
                          </dt>
                ...
                <Card className="rounded-[8px] border-0 bg-[var(--app-surface-solid)] shadow-none">
                  <CardContent className="p-5">
                    ...
                    <p className="mt-2 text-[12px] font-normal text-[var(--app-text-primary)]">
                      {"local_chaves" in property && property.local_chaves
                        ? property.local_chaves
                        : "Informação não disponível para este perfil."}
                    </p>
                  </CardContent>
                </Card>
              )}
                    ...
                    >
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <dl className="grid grid-cols-2 gap-2 text-[10px] font-light text-muted-foreground sm:grid-cols-4">
                          <div>
                            <dt>Retirada em</dt>
                            <dd className="mt-0.5 font-normal text-[var(--app-text-primary)]">
                              {formatDate(propertyKey.checked_out_at, true)}
                            </dd>
                          </div>
                          <div>
                            <dt>Devolução prevista</dt>
                            <dd className="mt-0.5 font-normal text-[var(--app-text-primary)]">
                              {formatDate(propertyKey.expected_return_at, true)}
                            </dd>
                          </div>
                          <div>
                            <dt>Cadastrada em</dt>
                            <dd className="mt-0.5 font-normal text-[var(--app-text-primary)]">
                              {formatDate(propertyKey.created_at, true)}
                            </dd>
                          </div>
                          <div>
                            <dt>Atualizada em</dt>
                            <dd className="mt-0.5 font-normal text-[var(--app-text-primary)]">
                              {formatDate(propertyKey.updated_at, true)}
                            </dd>
                          </div>
                          {propertyKey.holder_user_id && (
                            <div className="col-span-2 sm:col-span-4">
                              <dt>ID do responsável</dt>
                              <dd className="mt-0.5 break-all font-mono font-normal text-[var(--app-text-primary)]">
                                {propertyKey.holder_user_id}
                              </dd>
                            </div>
                          )}
            ...
            <TabsContent value="history" className="space-y-6">
              <div
                className={cn(
                  "grid gap-6",
                  normalizedResourcesAvailable && "lg:grid-cols-2",
                )}
              >
                <Card className="rounded-[8px] border-0 bg-[var(--app-surface-solid)] shadow-none">
                  <CardHeader>
                  ...
                  <CardContent>
                    {historyQuery.isLoading ? (
                      ...
                      <div className="rounded-[8px] bg-destructive/10 p-4">
                        <p className="text-[12px] font-normal text-destructive">
                      ...
                      <div className="space-y-4">
                        {historyQuery.data.map((event) => {
                          ...
                          return (
                            <article
                              key={event.id}
                              className="flex gap-3 rounded-[8px] bg-[var(--app-surface-soft)] p-3"
                            >
                              <span className="mt-1 h-2 w-2 shrink-0 rounded-[4px] bg-primary" />
                              <div className="min-w-0">
                                <div className="flex flex-wrap items-center gap-2">
                                  <p className="break-words text-[12px] font-normal">
                                    {event.title}
                                  </p>
                                  <Badge
                                    variant="secondary"
                                    className="rounded-[4px] border-0 text-[9px] font-light"
                                  >
                                    {event.type}
                                  </Badge>
                                </div>
                                {message && (
                                  <p className="mt-1 whitespace-pre-wrap text-[11px] font-light text-muted-foreground">
                                    {message}
                                  </p>
                                )}
                                <p className="mt-1 text-[10px] font-light text-muted-foreground">
                            ...
                            >
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2 text-[10px] font-light text-muted-foreground">
                                  <div>
                                    <dt>Responsável</dt>
                                    <dd className="mt-0.5 break-words font-normal text-[var(--app-text-primary)]">
                                      {movement.holder_name || "Não informado"}
                                    </dd>
                                  </div>
                                  <div>
                                    <dt>Origem</dt>
                                  ...
                                  <div>
                                    <dt>Destino</dt>
                                    <dd className="mt-0.5 break-words font-normal text-[var(--app-text-primary)]">
                                      {movement.to_location || "Não informado"}
                                    </dd>
                                  </div>
                                  <div>
                                    <dt>Ocorrência</dt>
                                    <dd className="mt-0.5 font-normal text-[var(--app-text-primary)]">
                                      {formatDate(movement.occurred_at, true)}
                                    </dd>
                                  </div>
                                  <div>
                                    <dt>Devolução prevista</dt>
                                ...
                                <div className="mt-2 space-y-1 break-all font-mono text-[9px] font-light text-muted-foreground">
                                  ...
                                  {movement.holder_user_id && (
                                    <p>
                                      Responsável: {movement.holder_user_id}
                                    </p>
                                  )}
                                </div>
                              </div>
