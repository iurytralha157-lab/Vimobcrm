...
export function DistributionQueueEditor({
    ...
    setFormData(prev => ({
      ...
      conditions: [
        ...prev.conditions,
        {
          ...
          type: prev.conditions.some(condition => condition.type === 'whatsapp_message_contains')
  ...
  const updateCondition = (id: string, updates: Partial<RuleCondition>) => {
    setFormData(prev => {
      ...
      const removedLastWhatsAppCondition = current?.type === 'whatsapp_message_contains'
        && updates.type !== undefined
        && updates.type !== 'whatsapp_message_contains'
        && !conditions.some(condition => condition.type === 'whatsapp_message_contains');
  ...
  const removeCondition = (id: string) => {
    setFormData(prev => {
      ...
      const conditions = prev.conditions.filter(c => c.id !== id);
      const removedLastWhatsAppCondition = removed?.type === 'whatsapp_message_contains'
        && !conditions.some(condition => condition.type === 'whatsapp_message_contains');
  ...
  const handleSave = async () => {
    ...
    const hasConfiguredWhatsAppMessageCondition = formData.conditions.some(condition =>
  ...
  const renderConditionValueSelector = (condition: RuleCondition) => {
    switch (condition.type) {
      ...
      case 'meta_form': {
        ...
        const unavailableFormIds = condition.values.filter(formId => !knownFormIds.has(formId));
        const visibleMetaForms = metaFormConfigs.filter(form => (
        ...
        return (
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1">
              {metaFormsLoading && visibleMetaForms.length === 0 && unavailableFormIds.length === 0 && (
                <span className="rounded-md bg-[var(--app-surface)] px-2 py-1 text-xs text-muted-foreground">
              ...
              {!metaFormsLoading && !metaFormsError && visibleMetaForms.length === 0 && unavailableFormIds.length === 0 && (
                <span className="rounded-md bg-[var(--app-surface)] px-2 py-1 text-xs text-muted-foreground">
              ...
              {visibleMetaForms.map(form => {
                ...
                const selected = condition.values.some(value => formValues.includes(value));
                const linkedToCurrentQueue = Boolean(queue?.id) && form.round_robin_id === queue?.id;
                const linkedToAnotherQueue = Boolean(form.round_robin_id) && !linkedToCurrentQueue;
                ...
                const toggleForm = () => {
                  ...
                  const newValues = selected
                    ? condition.values.filter(value => !formValues.includes(value))
                ...
                return (
                  <Badge
                    ...
                    title={linkedToAnotherQueue ? 'Este formulário já está vinculado a outra fila.' : undefined}
                    className={cn(
                      conditionOptionBadgeClass(selected),
                      'gap-1',
                      !canToggle && '!cursor-not-allowed opacity-60 hover:bg-[var(--app-surface-solid)]',
                    )}
                    onClick={toggleForm}
                    onKeyDown={(event) => {
              ...
              {unavailableFormIds.map(formId => (
                <Badge
                  ...
                  onClick={() => updateCondition(condition.id, {
                    values: condition.values.filter(value => value !== formId),
                  })}
                  onKeyDown={(event) => {
                    if (event.key === 'Enter' || event.key === ' ') {
                      event.preventDefault();
                ...
                >
                  {formId}
                  <span className="rounded bg-black/10 px-1 text-[10px] font-normal">Indisponível</span>
                </Badge>
              ))}
            </div>
          </div>
                  ...
                  {hasWhatsAppMessageCondition && (
                    <div className="flex items-start justify-between gap-4 rounded-lg bg-[var(--app-surface-soft)] p-3">
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Deixe desativado para respeitar os horários dos participantes adicionados como equipe. Corretores adicionados diretamente não possuem uma escala de equipe vinculada nesta fila.
                        </p>
                      </div>
                      <Switch
                        id="distribution-ignore-availability"
                        checked={!!formData.settings.ignore_availability}
              ...
              <Collapsible data-tour="distribution-queue-members" open={openSections.includes('members')} onOpenChange={() => toggleSection('members')}>
                ...
                <CollapsibleContent className="space-y-4 px-1 pt-4">
                  ...
                  {hasWhatsAppMessageCondition && !formData.settings.ignore_availability && (
                    <p className="text-xs text-muted-foreground">
                      Participantes adicionados como equipe respeitam a escala configurada. Corretores diretos continuam elegíveis porque não possuem uma escala de equipe vinculada nesta fila.
                    </p>
                  )}
