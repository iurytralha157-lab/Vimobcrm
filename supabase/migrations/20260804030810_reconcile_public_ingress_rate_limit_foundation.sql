-- Forward reconciliation for environments whose remote migration history had
-- already advanced past 20260729014500 when the shared ingress limiter was
-- introduced. Keep this migration idempotent so databases that did receive the
-- original foundation retain their counters and simply refresh the functions.
...
create index if not exists public_ingress_rate_limits_updated_idx
