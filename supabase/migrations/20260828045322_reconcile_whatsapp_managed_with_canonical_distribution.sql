-- Reconcile the campaign-specific WhatsApp allocator with the canonical
-- distribution boundary introduced in 20260728183851. Managed campaign leads
-- remain fail-closed, while every other immediate intake uses
-- private.distribute_lead instead of the retired legacy allocator.
do $$
begin
  if to_regprocedure(
...
begin
       ...
       and not exists (
         select 1
          ...
          where team_member.organization_id = queue.organization_id
            ...
            and team_member.user_id is null
  ...
  select
    ...
    left join public.team_members contextual_team_member
      on contextual_team_member.organization_id = member.organization_id
     and contextual_team_member.team_id = member.team_id
     and contextual_team_member.user_id = member.user_id
...
declare
  v_lead public.leads%rowtype;
  v_target_round_robin_id_text text;
...
begin
  ...
  if lower(btrim(coalesce(v_lead.metadata->>'managed_whatsapp_message_distribution', 'false')))
       in ('true', '1', 'yes') then
  ...
  if lower(btrim(coalesce(v_lead.metadata->>'distribution_deferred', 'false')))
       in ('true', '1', 'yes') then
    return jsonb_build_object(
      'handled', false,
      'success', false,
      'reason', 'distribution_deferred'
    );
  end if;

  v_target_round_robin_id_text := btrim(coalesce(v_lead.metadata->>'target_round_robin_id', ''));
  if v_target_round_robin_id_text ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' then
    v_target_round_robin_id := v_target_round_robin_id_text::uuid;
  end if;

  return private.distribute_lead(
    v_lead.organization_id,
    v_lead.id,
    'trigger:' || v_lead.id::text,
...
begin
  if new.assigned_user_id is null
     and (
       lower(btrim(coalesce(new.metadata->>'managed_whatsapp_message_distribution', 'false')))
         in ('true', '1', 'yes')
       or lower(btrim(coalesce(new.metadata->>'distribution_deferred', 'false')))
         not in ('true', '1', 'yes')
     ) then
