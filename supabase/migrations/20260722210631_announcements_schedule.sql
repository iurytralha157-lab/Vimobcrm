...
-- As interfaces de administração e do banner já leem e gravam estes campos.
...
drop policy if exists "vimob_canonical_9cc813bff9178df78090c772" on public.announcements;
...
using (
  exists (
    select 1
    from public.users super_admin_user
    where super_admin_user.id = (select auth.uid())
      and super_admin_user.role = 'super_admin'
  )
  or (
    is_active = true
    ...
    and (
      target_type = 'all'
      or (target_type = 'specific' and (select auth.uid()) = any(target_user_ids))
      or (
