...
type ImoviewIntegration = { organization_id: string };
...
async function authorizeScheduledRequest(req: Request): Promise<Response | null> {
  const expected = Deno.env.get("IMOVIEW_SCHEDULED_SYNC_SECRET")?.trim() ?? "";
  if (!expected) {
    console.error("[imoview-scheduled-sync] Scheduler secret is not configured");
...
async function syncOrganization(supabase: SupabaseClient, organizationId: string) {
  try {
    ...
    if (error) {
      console.error(`[imoview-scheduled-sync] Sync failed for org ${organizationId}:`, error);
      return;
    }
    console.log(
      `[imoview-scheduled-sync] Org ${organizationId} completed with ${data?.synced ?? 0} properties and ${data?.errors?.length ?? 0} errors`,
    );
  } catch (error) {
    console.error(`[imoview-scheduled-sync] Sync exception for org ${organizationId}:`, error);
  }
}

async function runWithConcurrency(
  supabase: SupabaseClient,
  integrations: ImoviewIntegration[],
): Promise<void> {
  let nextIndex = 0;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq("is_active", true);
    if (error) {
      console.error("[imoview-scheduled-sync] Could not list active integrations:", error);
      return jsonResponse({ ok: false, error: "integration_lookup_failed" }, 500);
    }

    const integrations = (data ?? []) as ImoviewIntegration[];
    if (integrations.length === 0) return jsonResponse({ ok: true, accepted: false, queued: 0 }, 200);

    const task = runWithConcurrency(supabase, integrations).catch((error) => {
      console.error("[imoview-scheduled-sync] Background batch failed:", error);
    });
    ...
    console.error("[imoview-scheduled-sync] Fatal error:", error);
