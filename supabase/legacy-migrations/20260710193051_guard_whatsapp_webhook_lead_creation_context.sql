-- Guard WhatsApp webhook lead creation so plain conversations never become leads.
...
as $$
  select coalesce(
    nullif(btrim(p_metadata #>> '{whatsapp_attribution,ctwa_clid}'), '') is not null
    or nullif(btrim(p_metadata #>> '{whatsapp_attribution,source_id}'), '') is not null
    or nullif(btrim(p_metadata #>> '{whatsapp_attribution,source_referral,ctwa_clid}'), '') is not null
    or nullif(btrim(p_metadata #>> '{whatsapp_attribution,source_referral,source_id}'), '') is not null
    or lower(coalesce(p_metadata #>> '{whatsapp_attribution,source_referral,explicit_source_type}', '')) in (
      'ad',
      'ads',
      'ctwa',
      'click_to_whatsapp',
...
grant execute on function public.whatsapp_webhook_has_lead_creation_context(jsonb) to service_role;
