...
begin
  if not exists (
    ...
    where c.conrelid = 'public.whatsapp_conversations'::regclass
      and c.conname = 'whatsapp_conversations_session_id_remote_jid_key'
      and c.contype = 'u'
  ) then
    raise exception using
      errcode = '55000',
      message = 'resolve_automation_whatsapp_conversation requires the unique session_id/remote_jid contract';
  end if;
end
$contract$;
